...
### レビューコメント（NFR2018観点）
- **セキュリティ**: 認証はMFA/パスワードポリシー、セッション期限、TLSバージョンを明示し、権限変更・閲覧・エクスポートを含む監査ログを1年以上保持し改ざん検知を設けることを推奨。暗号化（保存時/転送時）、鍵管理、脆弱性診断サイクルも要件化したい。
- **保守性・運用性**: SLI/SLO（API成功率・P95/99レスポンス）を設定し、モニタリング/アラート、IaCでの環境整合性、リリース/ロールバック手順、品質ゲート（自動テスト/セキュリティスキャン）を追記すると運用リスクを下げられる。
- **拡張性**: 外部連携や将来の保護者ポータルに備えAPIバージョニングと後方互換ルール、スキーマ移行手順（ゼロダウンタイム）の方針を追加。重い処理（AI解析・PDF生成）は非同期キュー化し、UIは進捗表示を行う旨を明記すると性能とユーザビリティが担保される。
