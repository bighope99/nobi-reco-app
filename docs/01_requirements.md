...
| **REC-03** | AI解析・抽出 | **新規作成時のみ**「AI作成」ボタン押下により実行：<br>1. メンションされた児童ごとのエピソード抽出<br>2. 「事実」と「所感」の分離<br>3. 文脈に基づく推奨タグの選択<br>※編集時はAI再解析を実行しない（事実・所感の直接編集のみ） | 高 |
| **REC-04** | 記録確認・承認 | AI生成結果（個別記録案）を一覧表示<br>人間が内容を確認・修正し、チェックボックスで承認・確定<br>確定時、活動記録と個別記録の両テーブルへ保存<br>**Human-in-the-loop**<br>※AI解析済みの記録編集時は、事実・所感を直接編集可能（本文は編集不可） | 高 |
...
```
1. AI解析済みの記録を開く
2. 本文（content）は非表示・編集不可
3. 事実（fact）・所感（comment）を直接編集
4. 保存（AI再解析は実行されない）
※ 追記が必要な場合は新規記録として別途作成する
```
