...
**処理内容**:
1. 画像ファイルを検証（形式、サイズ）
2. Supabase Storage（**非公開バケット**）にアップロード
3. 短期の署名付きURLを発行して返却（例: 5分有効）
4. URLは期限切れになるため、以降の表示はサーバーで再署名して提供
...
**アクセス制御**:
- 署名付きURLは短時間のみ有効
- 施設・ロールに紐づく権限制御（Storageポリシー/RLS）でアクセスを制限
...
> **重要: 編集時のAI再解析について**
>
> AI解析済み（個別記録が生成済み）の活動記録では、以下のルールが適用されます:
>
> - **本文（content）は編集不可**: AI解析済みの場合、活動記録の本文は非表示・編集不可となります。事実・所感の整合性を保つためです。
> - **事実・所感は直接編集可能**: AI解析で生成された個別記録の事実（fact）・所感（comment）は、編集画面から直接修正できます。
> - **AI再解析は実行されない**: 編集時にAI再解析は走りません。事実・所感が意図せず上書きされることを防止しています。
> - **追記が必要な場合**: 新規の活動記録として別途作成してください。
...
AI解析は**新規作成時のみ**実行されます。記録の編集時にはAI再解析は行いません。

```
【新規作成フロー】
1. 活動記録の本文入力 + @メンション
2. 「AI作成」ボタン → AI解析実行
3. 事実（fact）・所感（comment）・タグに自動分割
4. Human-in-the-loop: 確認・修正 → 承認・保存
...
> **設計理由**: 以前は編集時にもAI再解析が自動実行されていましたが、
> 職員が手動で修正した事実・所感がAIによって上書きされるバグがありました。
> この問題を解消するため、AI解析は新規作成時に限定し、
> 編集時は事実・所感の直接編集のみ許可する仕様としています。

### レポート生成時のデータソース

レポート生成（`POST /api/children/:id/report/generate`）では、
AI解析後に保存された**事実（fact）・所感（comment）のデータ**を使用します。
活動記録の本文（content）は参照しません。
これにより、レポートのデータソースが明確になっています。
