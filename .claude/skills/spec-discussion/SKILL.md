...
| 状況 | 召集するメンバー |
|------|----------------|
| UIが絡む・デザインが未確定 | PM + Designer（DesignerがHTMLモックを作成してからEngineerが実装） |
...
Agent tool:
  ...
  prompt: "あなたはPMです。spec-discussion-teamのPM役として動作します。
           ...
           実装フェーズでは worktree 作成（既存ブランチを削除してから git gtr new <ブランチ名> --base main --yes）・Engineer/ReviewerのSpawn・指示出し・Notionステータス更新を担当します。"
```
...
#### Designerが関与したチケット（UI変更あり）

| ステップ | 担当 | 内容 |
|---------|------|------|
| 1 | **PM** | worktree 作成（既存ブランチを削除してから `git gtr new <branch> --base main --yes`） |
| 2 | **PM** | Notion を「**進行中**」に更新 |
| 3 | **Designer** | 該当ページファイルに直接モックを書く（ダミーデータで描画）→ 編集ファイルパスを親Claudeへ返す |
| 4 | **親Claude** | ユーザーにモックを提示して確認（AskUserQuestion）<br>「モックを作成しました: `{ファイルパス}`<br>http://localhost:3000/... で確認できます。<br>このデザインで実装を進めますか？<br>1. 進める<br>2. 修正が必要（具体的に指示）」 |
| 5 | **Engineer** | モックのUIはそのまま残してデータフェッチ・ロジックを実装 → テスト → コミット |
| 6 | **Reviewer** | コードレビュー（`/pr-review`） |
| 7 | **Engineer** | 指摘修正 → `/create-pr` でPR作成 → **PR URLを親Claudeへ返す** |
| 8 | **PM** | Notion を「**レビュー依頼**」に更新（PR URL付き） |
| 9 | **Engineer** | CodeRabbitループ（最大3回） |

#### Designerが関与しないチケット（バグ修正・バックエンドのみ）
