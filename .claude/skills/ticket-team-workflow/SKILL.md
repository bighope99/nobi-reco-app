...
Phase 3: レビュー & PR（Coder + PM担当、大規模時はReviewer追加）
  通常: Coder → /pr-review実行 → 指摘修正 → PR作成 → CodeRabbitループ(最大3回) → PR URLをPMに報告
  大規模: Reviewer → /pr-review（複数視点） → 指摘をCoderに返す → Coder修正 → PR作成 → CodeRabbitループ → PR URLをPMに報告
  PM → 全チケットのNotionステータスを一括更新（スクリプト使用）
```
...
Phase 3:
  各Coderが /pr-review → /create-pr → PR URL を SendMessage(to: "PM") で報告
  PM → 全チケットのNotionステータスを一括更新
  PM → SendMessage(to: "Leader") で全完了を報告
