...
Phase 0: ユーザー特定（Leader担当）
  Leader → AskUserQuestion でユーザー名を確認
  Planner → Notion APIでユーザーID解決
  → 以降のチケット取得・更新で使用する USER_NAME / USER_NOTION_ID を確定
...
Leaderが `AskUserQuestion` ツールを使い、以下を聞く:

> 「あなたのNotionユーザー名を教えてください（担当者フィールドに表示される名前）」

### Step 2: Notion ユーザーID解決（Planner担当）

LeaderからユーザーMCPツールを使ってNotionのユーザー一覧を取得し、名前からIDを特定する。

```
mcp tool: notion-get-users
→ 返ってきたリストから USER_NAME に一致するユーザーのIDを取得
→ USER_NOTION_ID として保持
```

以降の全フェーズで `USER_NAME` / `USER_NOTION_ID` を参照する。
...
`--assignee-id` を指定すると「担当者が自分 OR 担当者が未設定」のチケットのみ取得する。
...
### Step 0: チケットステータスを「進行中」に更新（Planner担当）
